...
The extension supports prompt caching for models that advertise this capability (currently Anthropic Claude models). Prompt caching reduces costs and improves response times by caching stable request prefixes across turns.
...
- Adds up to four `cache_control` breakpoints when enabled (last tool, system prompt, first user message, rolling last text-bearing message)
