...
- 代理工具: 選擇加入的工具, Copilot 的代理可以呼叫它們讀取這個延伸模組的診斷和設定, 並在您接受確認卡片後逐個開關地變更設定 ([詳細資料](docs/zh-tw/getting-started.md#讓代理管理您的-litellm-設定))
...
- VS Code 1.136.0 或更新版本, 已安裝 GitHub Copilot Chat 延伸模組並登入
...
您的提示與完成內容只在 VS Code 與您設定的 LiteLLM 伺服器之間傳輸。聊天輪次會去到聊天選擇器所指模型所在的地方: 除非您選了自己的模型, 否則那就是內建的 Copilot 模型。

| 功能 | 送出的內容 | 發往 | 時機 |
| --- | --- | --- | --- |
| 聊天 | 您的提示、附件、工具定義 | 聊天選擇器所指的模型 | 每個輪次 |
| 內嵌補全 | 游標周圍的檔案內容 | `inlineCompletions.model` 背後的伺服器 | 輸入時自動送出; 預設關閉且需明確指定模型 |
| 提交訊息產生 | 已暫存或工作區的差異、未追蹤檔案的名稱、您最近五筆提交主旨 | `commitGeneration.model` 背後的伺服器 | 僅在您明確叫用時 |
| 快速修復 | 診斷訊息以及周圍的程式碼 | 聊天選擇器所指的模型; 聊天檢視無法作答時發往 `quickFix.model` | 當您選擇「修復」或「解釋」時 |

發往您的 LiteLLM 伺服器的請求計入與聊天相同的用量追蹤和預算警示。發往內建 Copilot 模型的輪次則不計入。

一個預設開啟的例外, 即模型目錄重新整理:

- 延伸模組約每週從 `https://openrouter.ai/api/v1/models` 取得一次公開、無需驗證的模型清單。該請求不帶提示、不帶用量, 也不帶任何關於您或您伺服器的資訊。
- 把 `litellm-vscode-chat.models.openRouterCatalog` 設為 `false` 可關閉重新整理與自動比對。明確的 `_openrouter_model` 指示詞繼續離線使用內建快照。
