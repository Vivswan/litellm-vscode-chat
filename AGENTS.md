...
Dashboard appearance is reviewed, not gated. `scripts/dev/render-dashboard.ts` screenshots any fixture in `scripts/dev/renderFixtures/` through headless Chrome (`--fixture <file> --out <png>`, `--clip-viewport` for a focused shot), and those screenshots are the input to human and model design review. There is no pixel baseline: the gate was removed because it priced every deliberate visual improvement as a 37-file baseline negotiation, and the dashboard is meant to evolve. The harness still enforces what it can prove - it renders under the shell's real CSP and fails on any violation, and it pins the scroll offset before a full-page capture so renders are reproducible. **No PNGs are tracked in git outside `assets/`**, and a format-check step fails the build if any appear.
