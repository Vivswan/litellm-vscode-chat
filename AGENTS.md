...
- **Mode → TTL matrix**: `off` = nothing cached; `chat` = eligible anchors 5m; `agent` = eligible head anchors (tools/system/firstUser) 1h, rolling 5m; `auto` = eligible stable anchors (tools/system/firstUser) 1h, rolling always 5m. Static anchors are eligible once they meet `minCacheTokens`; `tokenSizeAutoBreakpoint` is retained only for compatibility and no longer affects TTL selection.
- **`minCacheTokens`** (default 4096): a universal floor — any anchor whose estimated block size is below it is suppressed (the provider won't cache a prefix shorter than its minimum). Also handles the degenerate tiny/absent system prompt.
...
- **Vertex/Gemini `cache_control` incompatibility** (`cacheControlIncompatible`, **native Gemini only**): Google Gemini implements caching via a separate `CachedContent` handle, not inline `cache_control` markers. LiteLLM's `vertex_ai` adapter switches into cached-content mode on seeing **any** `cache_control` marker, but the request still carries tools + system instruction, which Gemini rejects ("Tool config, tools and system instruction should not be set in the request when using cached content") — a fatal 400. When this flag is set, `resolveCachePlan` early-returns the fully disabled plan (emitting no markers) and relies on Gemini's implicit server-side caching, which still produces cache reads with zero markers sent. This check runs before the `supportsPromptCaching` gate so the disabled plan still preserves `mode` for diagnostics. **Scoping matters:** `vertex_ai` is LiteLLM's provider name for *everything* on Vertex, including Anthropic Claude (`vertex_ai/claude-*`), Llama and Mistral, which use the same inline `cache_control` semantics as direct Anthropic and get **no** implicit caching. `isGeminiCacheIncompatible` (in `registration.ts`) therefore requires a Gemini signal — a `gemini` provider name, or a `vertex` provider **with** `gemini` in the model id — so Claude/Llama/Mistral on Vertex keep their inline caching and only native Gemini is disabled.

Both flags are aggregated across a model's providers with `.some(...)`: a heterogeneous model group (e.g. a `:cheapest` route fronting several backends) degrades to the **most-restrictive** provider's cache behavior — if any member is Bedrock the tools-1h drop applies to the whole group, and if any member is native Gemini inline caching is disabled for the whole group. This is the safe default (it never emits a marker a member would reject), and the `toolsCache1hUnsupported` / `cacheControlIncompatible` values are logged so the chosen behavior is debuggable.
