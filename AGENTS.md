...
- `src/extension.ts` + `src/extension/`: activation, commands, status bar, notifier, diagnostics, and server storage (`serverRegistry.ts`: servers in `globalState` as a version-stamped blob, API keys in SecretStorage).
...
- **Capabilities vs parameters.** Capabilities (what a model can do) are read from the LiteLLM API and drive registration and token constraints. Parameters (what we ask it to do) come only from user config and runtime options. `modelParameters` uses longest-prefix matching and supports `ServerLabel/model` scoping.
...
- **Multi-server IDs.** With one server a model's exposed ID is the raw LiteLLM ID; with several it is `<serverId>/<rawModelId>`. A route map takes chat requests to the right server.
