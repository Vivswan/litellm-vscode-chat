...
延伸模組絕不替您決定請求參數: 除了它擁有的欄位 (模型、訊息、串流管線、`max_tokens` 與工具接線) 之外, 只有您在某處設定的參數會送達 LiteLLM, 而且原封不動地送達。本頁涵蓋可以設定參數的各個位置, 以及多處同時比對到同一個請求時它們如何組合。參數是請求要求什麼; 模型*能做什麼* (上下文長度、視覺、工具支援) 是[模型能力](model-capabilities.md)的職責。
...
唯一有文件說明的例外是 `max_tokens`: 當沒有任何來源設定它時, 延伸模組會送出您伺服器在模型資訊中宣告的輸出上限, 或在伺服器未宣告時最多送出 4096。在 [`modelCapabilities`](model-capabilities.md) 中設定的 `max_output_tokens` 算作已宣告的上限, 會原樣送出。
