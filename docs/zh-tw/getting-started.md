...
- **VS Code 1.129.0 或更高版本**, 已安裝並登入 GitHub Copilot Chat 延伸模組。本延伸模組接入的是 Copilot 的聊天檢視, 沒有它就沒有聊天介面, 也沒有模型選擇器。
...
3. 填寫表單:
   ...
   - **基底 URL** - 伺服器的根 URL, 例如 `http://localhost:4000`。不要寫 `/v1` 結尾; 延伸模組會自行附加。
   - **驗證** - 恰好一種形式: API 金鑰 (最常見的情況)、OAuth 用戶端認證, 或自訂標頭中的金鑰。對於金鑰, 表單的「安全儲存」選項會把它放進 VS Code [祕密儲存體](servers.md#祕密與祕密儲存體)而不是您的設定檔 - 這是預設選項, 也是任何您不會提交進儲存庫的值的正確選擇。
...
結尾的 `*` 讓鍵成為家族比對器。每個相符的鍵都會套用, 而對每個單獨的欄位, 最具體的比對勝出 - 所以 `gpt-5-turbo` 得到 0.3, `claude-4` 得到 0.7。詳情: [模型: 參數](models.md#參數)與[模型比對](models.md#模型比對)。
