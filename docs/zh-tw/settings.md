...
設定同步有意跳過這裡最要緊的部分 - `servers` 是機器範圍的, 祕密儲存體中的值也從不同步 - 所以把配置搬到另一台機器有自己的一對命令。儀表板的設定分頁以「匯入與匯出」按鈕承載它們; 命令或按鈕, 流程相同。
