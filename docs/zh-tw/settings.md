...
| `litellm-vscode-chat.chat.timeout` | `300000` | 單次聊天補全呼叫, 以及單次提交訊息產生、提取要求描述產生、諮詢工具或快速修復呼叫的硬性時間預算, 毫秒。聊天請求從不重試, 所以這是一個請求可占用的總時間, 含串流。最小 1000; 更低的值會被箝制。為長推理運行或緩慢的基礎設施調大它 |
