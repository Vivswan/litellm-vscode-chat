...
- On an older VS Code, update it: the extension needs 1.136.0 or higher.
...
- **The extension has no idle timeout of its own.** A model may pause between chunks, or before its first byte, for as long as `chat.timeout` allows; the extension never closes the connection for silence alone. A proxy, gateway, or server between VS Code and the model may still enforce its own idle limit. The same holds for the commit, pull request, consult, quick fix, and review calls, which share that budget.
...
- **"A fresh machine has my model settings, but no models."** The flip side of the same rule: Settings Sync delivers your global settings - `models.parameters`, `models.capabilities`, usage thresholds - but no `servers` (and no feature toggles or model picks, which are machine-overridable and stay per machine), so there is nothing yet for the records to apply to. Re-add the servers and the synced records take effect on their own as matching models register. Anything that lives on an entry (credentials, per-server records, headers, the `budget` field) needs re-entering with it.
...
- **"Where should a credential live, then?"** In a server entry - and only there. The entry's [`auth` forms](servers.md#authentication) (which can use secret storage) and its per-entry `headers` object are both machine-scoped, so a key can neither replicate through Settings Sync nor land in a committed `.vscode/settings.json`. Other settings (`models.parameters`, timeouts, usage thresholds) sync normally and carry nothing credential-like; the feature toggles and model picks stay per machine ([scope](settings.md#how-settings-work)).
...
The dashboard puts a diagnostic line under a server's row - "may not be applying its per-server model parameters" - when the entry carries per-entry `models.parameters` but the VS Code provider group serving that server may not carry the entry's labeled identity. That happens when the group predates entry labels, or when a rename or base URL edit left a stale group behind; requests through such a group may get only the global `models.parameters` setting.

The same line names every other affected surface too: an entry's `models.capabilities`, `discovery.declared`, and `discovery.expectedFailures`, its custom `headers`, and its `apiVersion` override (requests fall back to the auto rule); all have the same fixes.
...
If a setting seems to have vanished after an upgrade, it was renamed or restructured: a one-time migration moves everything automatically -

- setting names into their new sections (`requestTimeout` to `chat.timeout`, `modelParameters` to `models.parameters`, and so on),
- flat server entries into the `auth`/`models`/`discovery` shape,
- the `default*` token settings into a `models.capabilities` `"*"` record,
- the `_declare` directive into the entry's `discovery.declared` list,
- and the old global `headers` setting into each entry's own `headers` object (a copy no entry can receive is left in place, inert, with a dashboard hint).

Nothing needs re-entering; the [rename table](settings.md#renamed-and-removed-settings) maps every old name to its new home.
...
Your prompts, attachments, and completions travel only between VS Code and the endpoints you configure - plus, for anything sent as a chat turn, wherever the model the chat picker names goes, which is a built-in Copilot model unless you selected one of yours:

- Chat and model-discovery requests go to your LiteLLM servers' base URLs.
- Commit message generation, on your explicit invocation only, sends the staged diff - or, when nothing is staged, the working-tree diff plus the paths of untracked files, never their contents - and your last five commit subjects to the LiteLLM server you configured for it. The dashboard's "Test model" button sends one small fixed sample diff with canned style subjects, never your repository ([Getting started](getting-started.md#generate-commit-messages-with-your-own-model)).
- Pull request description generation, on your explicit invocation only, sends your branch name, its commit messages, and one patch per changed file (compared from the merge base with the branch it would be merged into) to the LiteLLM server you configured for it.
  - Through the generate button the feature registers in the GitHub Pull Requests extension's Create Pull Request view, that extension assembles the context instead, and it adds two things: your repository's pull request template, and the title and body of every issue your commit messages reference - including private ones.
  - The dashboard's "Test model" button sends one small fixed sample branch, never your repository ([Getting started](getting-started.md#generate-pull-request-descriptions-with-your-own-model)).
...
- The agent tools, while enabled, hand a read tool's output - server labels, base URLs, model IDs, effective capabilities, and log lines redacted the way the issue report redacts them - to whatever model the chat agent runs on - a cloud model, or one of your own LiteLLM models if the picker names one.
  - A write tool changes your settings on this machine, after you accept a confirmation card. A write that touches a `servers` entry then makes the same discovery and usage requests to that host that saving in the dashboard makes.
  - `litellm_run_action`'s connection test probes a stored server, and its feature-model test sends a fixed probe prompt to the feature's picked model - a model request, billed like any other. Its sync and refresh actions reach only the hosts in your configuration and the catalog URL ([Getting started](getting-started.md#let-an-agent-manage-your-litellm-setup)).
- Quick fixes, while enabled and only when you pick Fix or Explain from the lightbulb, send the diagnostic messages and the lines they sit on (plus two lines either side).
  - On the chat path that is an ordinary `@litellm` turn, so it goes to whichever model the chat picker names - one of your LiteLLM models if you selected one, a built-in Copilot model otherwise - and, like any chat turn, earlier messages in that conversation ride along as context.
  - When the chat view cannot answer, the fallback sends the same material to the server behind `quickFix.model` instead; the dashboard's "Test model" button on that row sends one small fixed code snippet, never your files ([Getting started](getting-started.md#fix-or-explain-a-diagnostic)).
- Review comments, on your explicit invocation only, send the code being reviewed to the LiteLLM server you configured for them: in changes mode each reviewed file's diff against HEAD, in file mode that whole file's content, in both cases with the file's path relative to the workspace or repository (just the file name for a file that belongs to neither, so no absolute path leaves the machine), and in file mode its VS Code language identifier.
  - Replying in a thread sends that thread's conversation plus the lines it is anchored to.
  - Nothing is reviewed automatically; the dashboard's "Test model" button sends one small fixed sample diff, never your files ([Getting started](getting-started.md#get-review-comments-on-your-code)).
