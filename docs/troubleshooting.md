...
- **Silence is not a failure.** A model may pause between chunks, or before its first byte, for as long as `chat.timeout` allows; the connection is never closed for silence alone. The same holds for the commit, pull request, consult, quick fix, and review calls, which share that budget.
