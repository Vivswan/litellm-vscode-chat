...
- Pull request description generation, on your explicit invocation only, sends your branch name, its commit messages, and one patch per changed file (compared from the merge base with the branch it would be merged into) to the LiteLLM server you configured for it. Through the generate button the feature registers in the GitHub Pull Requests extension's Create Pull Request view, that extension assembles the context instead, and it adds two things: your repository's pull request template, and the title and body of every issue your commit messages reference - including private ones. The dashboard's "Test model" button sends one small fixed sample branch, never your repository ([Getting started](getting-started.md#generate-pull-request-descriptions-with-your-own-model)).
...
- Quick fixes, while enabled and only when you pick Fix or Explain from the lightbulb, send the diagnostic messages and the lines they sit on (plus two lines either side). On the chat path that is an ordinary `@litellm` turn, so it goes to whichever model the chat picker names - one of your LiteLLM models if you selected one, a built-in Copilot model otherwise - and, like any chat turn, earlier messages in that conversation ride along as context. When the chat view cannot answer, the fallback sends the same material to the server behind `quickFix.model` instead; the dashboard's "Test model" button on that row sends one small fixed code snippet, never your files ([Getting started](getting-started.md#fix-or-explain-a-diagnostic)).
- Review comments, on your explicit invocation only, send the code being reviewed to the LiteLLM server you configured for them: in changes mode each reviewed file's diff against HEAD, in file mode that whole file's content, in both cases with the file's path relative to the workspace or repository (just the file name for a file that belongs to neither, so no absolute path leaves the machine), and in file mode its VS Code language identifier. Replying in a thread sends that thread's conversation plus the lines it is anchored to. Nothing is reviewed automatically; the dashboard's "Test model" button sends one small fixed sample diff, never your files ([Getting started](getting-started.md#get-review-comments-on-your-code)).
