...
- Five destinations:
  - **Servers** is your entries, their health, and their spend against budgets, with each server's problems written under the row they belong to (except the warning-tier budget note, which waits in the row's drawer beside the spend facts - the row's tinted meter already signals it) and its full inventory, including the entry's own model records, behind the row's disclosure.
  - **Models** is everything those servers report, and a server drawer's model count navigates here scoped to it.
  - **Features** holds the extension's features beyond chat - inline completions, commit message generation, PR description generation, the consult tool, quick fixes, review comments, the chat participant, and the agent tools - each with its enable, model pick, and feature-specific rows ([Features](#features)).
  - **Settings** holds the rest of the extension's settings as editable form controls.
  - **Diagnostics** is the configuration problems the extension has spotted, [Resolved models](#resolved-models), and the support tools and links.
...
A row's problems render directly under it, worst first. Each line leads with the consequence ("prod is serving no models: ..."), carries the server's own error text where there was any, and offers the matching actions in place - Retry, Open entry, Fix in settings.json, Declare models, Open models file, Refresh now, or a troubleshooting link.

Usage problems rank on the same scale: a key denied its own usage data and a budget past its thresholds are counted problems, while a transient refresh failure is a quiet advisory that still renders in full (headline, technical detail, and Refresh now) because it clears itself on the next poll. When any row needs action, the heading's summary counts it ("2 need attention"); quiet advisory lines state facts that need none and stay out of that count. [Troubleshooting](troubleshooting.md#common-issues) covers the recovery steps.

One deliberate softening: an expected discovery failure that is still serving - the entry's [declared models](servers.md#declared-models), or the last known list inside the stale window - is not a problem: the row stays Connected, with a quiet advisory line stating the expected failure. With nothing serving at all the row goes red like any other server serving no models, and its verdict reads "Expected failure".
...
Add and Edit open the same form as a destination in the main pane, with the rail still beside it: one flat page, every part of the entry in the same scroll under an in-flow heading - Connection, Authentication, Model parameters, Model capabilities, Discovery, Headers and budget, MCP - with nothing folded away behind a disclosure.

It writes the `servers` setting, so edits made here and edits made in settings.json are the same thing; the form's value is that it enforces the entry's shape as you type instead of after you save.

Each field carries its hint beside it, an error takes the hint's place (so nothing shifts as you type), and the sticky bar at the bottom names the setting the entry lands in and counts what is still unsaved.

Leaving - the trail back at the top, the rail, Esc, or Discard changes - is a request rather than an act: with unsaved edits it asks, and only the explicit Discard destroys the draft. Focus follows you in and back out again, to the row you opened.
...
**MCP** - its own section: a switch that makes this server's own [MCP tools](servers.md#mcp-tools) available in chat, plus the endpoint to publish. Left empty, the hint names the exact address derived from the base URL.
...
Spend against budget lives on each server's row: the collapsed row carries the percentage and meter, and opening the row lists every number the extension holds - the effective and key-reported budgets when they differ, the budget's next reset date, and, where the server serves daily activity data, request count, success rate, and cache hit rate. A field the server does not report renders as a dim dash plus the reason in place, never as a zero.

Servers whose LiteLLM tracks no spend (no database) simply show an empty spend cell; a key denied its usage data keeps its row and states the denial as a problem line under it.

The heading's Refresh now button fetches immediately (disabled while a fetch is in flight); stale data stays visible, marked with its cause when one is known ("stale" beside the number, or a problem line naming the failed refresh or the denial). When an explicit refresh fails on every server, one warning toast says so; the per-server detail stays on the rows.

Opening the dashboard fetches usage data only when a fetch is due - no completed pass this session, the last one older than the poll interval, or a changed `servers` setting; otherwise the stored numbers just render. The depth - what the extension reads and from where, how the two budget sources interact, polling and freshness, alert thresholds, the status bar item - lives on the [Usage and budgets](usage.md) page.

## Features

The Features section is one page for everything the extension does beyond chat, one group per feature under a shared page filter. Every control is an ordinary setting ([the reference](settings.md#reference)), so the page and settings.json are two views of the same file.

- **Inline completions**, **Commit message generation**, **PR description generation**, the **Consult tool**, **Quick fixes**, **Review comments**, the **Chat participant (@litellm)** and the **Agent tools** are the shipped features: each carries its enable checkbox, its model picker where it has one, and its own rows - the inline language filter's mode and list, the commit prompt.
  - **Agent tools** has no model row: its enable switch registers the read tools, and six sub-switches follow it - one per write tool (set setting, edit model records, save server, remove server, run action) and one that lets tool input carry secret values. What each tool does and who may switch them is on the [recipe](getting-started.md#let-an-agent-manage-your-litellm-setup).
  - The model picker offers "Not set" plus every declared entry's served (server, model) pair, and "Custom model ID..." for models discovery cannot list (completion-mode FIM models above all).
  - The inline, commit, PR, consult, quick-fix and review model rows carry **Test model**: one probe running the exact pipeline the feature runs - ghost text, a fixed sample diff for a commit message, a fixed sample branch, a fixed question, the quick-fix fallback over a fixed sample diagnostic, or a small fixed sample diff - answering with counts or the classified error, never the model's own text.
  - The quick-fix model backs only the fallback path; its chat path, like the participant, uses whichever model the chat picker names, which is why neither has a model row that decides it.
- A model pick names a declared entry's label plus a raw model ID; external groups have no entry label, so their models are never offered ([feature model settings](settings.md#reference)).

## Settings

The Settings section renders the settings the native Settings editor shows, in its section order (Models, Chat, Discovery, Usage, UI; `servers` carries no scalar settings and is managed on the [Servers](#servers) page and in the [server form](#the-server-form), and the per-feature settings live on [Features](#features)), as form rows with their units, defaults, and a Reset action. A configured row says where its value lives ("Modified in User settings"); Reset removes the value from that scope so the next scope's value or the default shows through.

- The object-shaped settings - [`models.parameters`](settings.md#reference) and [`models.capabilities`](settings.md#reference) - get matcher tables here, since the native settings GUI cannot edit objects.
  - They use the same table as the [server form](#the-server-form)'s per-server sections: one row per matcher, sorted broadest first (the catch-all at the top, exact IDs at the bottom - a display order only; the stored key order never changes), fields as chips with force/fallback/inheritable badges, chip popovers for quick edits, and a pencil opening the full matcher editor.
  - Capability field names are free-form, with the same advisory typo hint as the server form's table. Apply stays disabled until every field parses.
...
- The page header's actions are the support tools: **Test connection**, **Open output log**, **Copy diagnostics**, and **Report a bug**. The per-server connection facts an older version summarized here now live on each server's row on the [Servers](#servers) page; Copy diagnostics still collects them into one plain-text block for a bug report - the rail's verdict, the configured-server count, the last check as an absolute timestamp, and one outcome line per server ("OK (12 models)", the error text otherwise). The block also carries the configuration-diagnostics lines and, when any provider group is hidden, a "Hidden provider groups" count.
