...
- Four destinations. **Servers** is your entries, their health, and their spend against budgets, with each server's problems written under the row they belong to (except the warning-tier budget note, which waits in the row's drawer beside the spend facts - the row's tinted meter already signals it) and its full inventory, including the entry's own model records, behind the row's disclosure. **Settings** holds the extension's settings as editable form controls. **Models** is everything those servers report, and a server drawer's model count navigates here scoped to it. **Diagnostics** is the configuration problems the extension has spotted, [Resolved models](#resolved-models), and the support tools and links.
