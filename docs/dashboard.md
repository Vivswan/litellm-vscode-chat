...
- **Inline completions** and **Commit message generation** are the shipped features: each carries its opt-in checkbox, its model picker, and its own rows - the inline language filter's mode and list, the commit prompt. The model picker offers "Not set" plus every declared entry's served (server, model) pair, and "Custom model ID..." for models discovery cannot list (completion-mode FIM models above all). The inline model row carries **Test model**: one probe running the exact pipeline ghost text runs, answering with counts or the classified error, never completion text.
- **PR description generation**, **Consult tool**, **Quick fixes**, **Review comments**, and the **Chat participant (@litellm)** are registered ahead of their features: their headings carry a quiet "Coming soon" badge, one hint at the top of the page says what it means, and the settings exist and persist (enable them early if you like) and take effect when each feature ships. The participant has no model row - it answers with the chat request's own model.
