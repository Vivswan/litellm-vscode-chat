...
- **Inline completions**, **Commit message generation**, **PR description generation**, the **Consult tool**, and the **Chat participant (@litellm)** are the shipped features: each carries its enable checkbox, its model picker where it has one, and its own rows - the inline language filter's mode and list, the commit prompt. The model picker offers "Not set" plus every declared entry's served (server, model) pair, and "Custom model ID..." for models discovery cannot list (completion-mode FIM models above all). The inline, consult and PR model rows carry **Test model**: one probe running the exact pipeline the feature runs - ghost text for one, a fixed question for the next, a canned sample branch for the last - answering with counts or the classified error, never the model's own text. The participant has no model row: it answers with the chat request's own model.
- **Quick fixes** and **Review comments** are registered ahead of their features: their headings carry a quiet "Coming soon" badge, one hint at the top of the page says what it means, and the settings exist and persist (enable them early if you like) and take effect when each feature ships.
