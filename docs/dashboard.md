...
- *Model capabilities for this server*: the same table for capability overrides. Capability chips carry a **fallback** toggle - a fallback field applies *below* what the server reports instead of above it - and the `_openrouter_model` directive renders as a catalog chip whose editor is the OpenRouter catalog picker ([capabilities](models.md#capabilities)).
