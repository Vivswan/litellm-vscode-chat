...
- *Model parameters for this server*: rows keyed by [matcher](models.md#model-matching), values as JSON (`0.2`, `true`, `["stop"]`), validated per row. Each row has a **Force** checkbox - a forced field beats the chat client's runtime options and the model picker's configuration ([parameters](models.md#parameters)). Force is disabled, with the reason, on provider-owned keys (`model`, `messages`, `stream`, ...), which the extension never lets anything override.
- *Model capabilities for this server*: the same row editor for capability overrides. Each row has a **Fallback** checkbox - a fallback field applies *below* what the server reports instead of above it - and an OpenRouter catalog picker for the `_openrouter_model` directive ([capabilities](models.md#capabilities)).
...
- Configuration problems in the matched records - unknown fields, invalid values, an invalid regex matcher, an `_openrouter_model` ID the catalog does not know - are called out here, beside the rows they affect.
...
- The object-shaped settings - [`models.parameters`](settings.md#reference) and [`models.capabilities`](settings.md#reference) - get row editors here, since the native settings GUI cannot edit objects. They use the same editor as the [server form](#the-server-form)'s per-server sections: matcher keys, JSON values with per-row validation (Apply stays disabled until every row parses), and the per-row Force / Fallback checkboxes. (Custom HTTP headers are per server, not global: they live on the entry and are edited in the [server form](#the-server-form).)
