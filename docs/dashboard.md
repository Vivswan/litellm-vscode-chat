...
- Five destinations. **Servers** is your entries, their health, and their spend against budgets, with each server's problems written under the row they belong to (except the warning-tier budget note, which waits in the row's drawer beside the spend facts - the row's tinted meter already signals it) and its full inventory, including the entry's own model records, behind the row's disclosure. **Models** is everything those servers report, and a server drawer's model count navigates here scoped to it. **Features** holds the extension's features beyond chat - inline completions, commit message generation, PR description generation, the consult tool, quick fixes, the chat participant, and the upcoming ones - each with its enable, model pick, and feature-specific rows ([Features](#features)). **Settings** holds the rest of the extension's settings as editable form controls. **Diagnostics** is the configuration problems the extension has spotted, [Resolved models](#resolved-models), and the support tools and links.
...
The server list shows every server the extension knows about: entries declared in the [`litellm-vscode-chat.servers` setting](servers.md#entry-reference), and "external" servers that exist only as VS Code provider groups (added outside this extension - see [adoption](servers.md#external-servers-and-adoption)). Groups hidden by a removed entry fold into a "hidden groups" line with an Unhide action ([lifecycle](servers.md#lifecycle-renames-removals-hidden-groups)). The section heading carries a one-line summary - the server count, how many rows need attention, the worst budget use among fresh rows (marked "stale rows excluded" when a stale figure is visible below it), and whether background polling is off - plus Add server and a Refresh now button that fetches usage data immediately (disabled while a fetch is in flight).
...
Spend against budget lives on each server's row: the collapsed row carries the percentage and meter, and opening the row lists every number the extension holds - the effective and key-reported budgets when they differ, the budget's next reset date, and, where the server serves daily activity data, request count, success rate, and cache hit rate. A field the server does not report renders as a dim dash plus the reason in place, never as a zero. Servers whose LiteLLM tracks no spend (no database) simply show an empty spend cell; a key denied its usage data keeps its row and states the denial as a problem line under it. The heading's Refresh now button fetches immediately (disabled while a fetch is in flight); stale data stays visible, marked with its cause when one is known ("stale" beside the number, or a problem line naming the failed refresh or the denial). When an explicit refresh fails on every server, one warning toast says so; the per-server detail stays on the rows.
...
- **Inline completions**, **Commit message generation**, **PR description generation**, the **Consult tool**, **Quick fixes** and the **Chat participant (@litellm)** are the shipped features: each carries its enable checkbox, its model picker where it has one, and its own rows - the inline language filter's mode and list, the commit prompt. The model picker offers "Not set" plus every declared entry's served (server, model) pair, and "Custom model ID..." for models discovery cannot list (completion-mode FIM models above all). The inline, PR, consult and quick-fix model rows carry **Test model**: one probe running the exact pipeline the feature runs - ghost text, a fixed sample branch, a fixed question, or the quick-fix fallback over a fixed sample diagnostic - answering with counts or the classified error, never the model's own text. The quick-fix model backs only the fallback path; its chat path, like the participant, uses whichever model the chat picker names, which is why neither has a model row that decides it.
- **Review comments** is registered ahead of its feature: its heading carries a quiet "Coming soon" badge, one hint at the top of the page says what it means, and the settings exist and persist (enable them early if you like) and take effect when the feature ships.
