...
- Four destinations. **Servers** is your entries, their health, and their spend against budgets, with each server's problems written under the row they belong to and its full inventory behind the row's disclosure. **Models** is everything those servers report, and a server drawer's model count navigates here scoped to it. **Diagnostics** is the configuration problems the extension has spotted, the resolved-records views, and the support tools and links. **Settings** holds the extension's settings as editable form controls.
