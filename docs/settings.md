...
| `litellm-vscode-chat.agentTools.saveServer.enabled` | `false` | Lets the agent add, edit, rename, or adopt a `servers` entry. You type secret values into a masked box unless `agentTools.secretValues.enabled` lets the agent pass them; a kept secret never follows a changed host. Needs `agentTools.enabled`. Recipe: [Let an agent manage your LiteLLM setup](getting-started.md#let-an-agent-manage-your-litellm-setup) |
...
| `litellm-vscode-chat.agentTools.secretValues.enabled` | `false` | Lets agent tool input carry secret values (API keys, client secrets). Off, the agent only chooses where a secret is stored and VS Code asks you to type it. Turn on only where the chat transcript is already a secure place, such as a zero-retention deployment. Recipe: [Let an agent manage your LiteLLM setup](getting-started.md#let-an-agent-manage-your-litellm-setup) |
