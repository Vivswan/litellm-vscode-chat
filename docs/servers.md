...
| Remove an entry | The group cannot be removed, so the extension *hides* it: remembers the removal, answers the group with an empty model list (models leave the picker), and folds the row into the dashboard's "hidden groups" line. That line offers Unhide and the deletion routes: the models file for any group, and Manage Language Models (its Delete action) for a group the extension created. The removal notice offers the same. Reload the window after deleting; the dashboard remembers the group until then. The identity the extension hides is the one the host actually served the label at, so a group that never synced cleanly (a blocked entry) is hidden too |
...
- **Remove** hides the group, same as removing a declared entry. The hidden-groups line then offers the deletion routes: the models file for any group (the object with the row's base URL), and Manage Language Models, searched for the name the sync gave the group, when the extension created it. The host never hands over the name of a group added outside the extension.
