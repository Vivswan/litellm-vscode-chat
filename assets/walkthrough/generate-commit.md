...
Once both are set, a sparkle button appears in the Source Control title bar (and the command palette gains "LiteLLM: Generate Commit Message"). It sends your staged diff - or the working-tree diff plus untracked file names when nothing is staged - to the model and writes the drafted message into the commit box.
