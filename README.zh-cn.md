...
- 代理工具: 选择加入的工具, Copilot 的代理可以调用它们读取这个扩展的诊断和配置, 并在你接受确认卡片后逐个开关地修改设置 ([详情](docs/zh-cn/getting-started.md#让代理管理你的-litellm-设置))
...
- VS Code 1.136.0 或更高版本, 已安装并登录 GitHub Copilot Chat 扩展
...
你的提示和补全只在 VS Code 与你配置的 LiteLLM 服务器之间传输。聊天轮次会去到聊天选择器所指模型所在的地方: 除非你选了自己的模型, 否则那就是内置的 Copilot 模型。

| 功能 | 发送的内容 | 发往 | 时机 |
| --- | --- | --- | --- |
| 聊天 | 你的提示、附件、工具定义 | 聊天选择器所指的模型 | 每个轮次 |
| 内联补全 | 光标周围的文件内容 | `inlineCompletions.model` 背后的服务器 | 输入时自动发送; 默认关闭且需显式指定模型 |
| 提交消息生成 | 已暂存或工作区的差异、未跟踪文件的名称、你最近五条提交主题 | `commitGeneration.model` 背后的服务器 | 仅在你显式调用时 |
| 快速修复 | 诊断消息以及周围的代码 | 聊天选择器所指的模型; 聊天视图无法作答时发往 `quickFix.model` | 当你选择「修复」或「解释」时 |

发往你的 LiteLLM 服务器的请求计入与聊天相同的用量跟踪和预算警报。发往内置 Copilot 模型的轮次则不计入。

一个默认开启的例外, 即模型目录刷新:

- 扩展约每周从 `https://openrouter.ai/api/v1/models` 获取一次公开的、无需身份验证的模型列表。该请求不携带提示、不携带用量, 也不携带任何关于你或你的服务器的信息。
- 把 `litellm-vscode-chat.models.openRouterCatalog` 设为 `false` 可关闭刷新和自动匹配。显式的 `_openrouter_model` 指令继续离线工作于内置快照。

详情见[模型能力](docs/zh-cn/models.md#openrouter-目录)和[隐私与数据](docs/zh-cn/troubleshooting.md#隐私与数据)。
