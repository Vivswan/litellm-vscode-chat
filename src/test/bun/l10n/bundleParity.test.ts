...
function bundleLocales(): Map<string, string> {
	return localeFiles(path.join(REPO_ROOT, "l10n"), /^bundle\.l10n\.([\w-]+)\.json$/);
}

function nlsLocales(): Map<string, string> {
	return localeFiles(REPO_ROOT, /^package\.nls\.([\w-]+)\.json$/);
}
...
describe("l10n drift guard: translation-file parity", () => {
