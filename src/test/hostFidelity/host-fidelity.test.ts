...
suite("Host-Fidelity Tests (group label round trip)", () => {
	...
	test("the host hands configuration.label back; same-URL same-key groups keep distinct identities", async function () {
		this.timeout(30000);
		// Last suite in this file on purpose: provider groups are add-only for
		// the host lifetime, so the groups created here linger, and the earlier
		// suites' exact-set model waits must never see their models.
