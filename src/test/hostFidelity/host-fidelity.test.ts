...
/**
 * Groups are add-only for the host lifetime, so each suite mints per-group-unique model IDs and scopes its
 * model-list assertions to that universe. Live mode is an explicit opt-in (LITELLM_REAL_LIVE=1) so ambient
 * LITELLM_REAL_* vars cannot silently turn `bun run test` into a live run with the capture suites collapsed to
 * skip stubs.
 */
