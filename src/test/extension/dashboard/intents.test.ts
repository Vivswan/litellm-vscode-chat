...
suite("extension/dashboard/intents", () => {
	...
	suite("executeDashboardIntent: resolveParkedHeaders", () => {
		const parked = (headers: Record<string, unknown>) => ({ headers, migratedAt: 1 });

		test("apply merges the parked headers onto the named entry and deletes the parked record", async () => {
			const recorded = makeEnv([{ label: "Prod", baseUrl: "http://prod.test", junk: "kept" }]);
			recorded.parkedGlobalHeaders = parked({ "x-env": "gold", "x-count": 3, "x-flag": true });

			const notice = await executeDashboardIntent(
				{ method: "resolveParkedHeaders", payload: { action: "apply", label: "Prod" } },
				recorded.env
			);

			assert.strictEqual(notice, undefined, "a full apply acks quietly");
			assert.deepStrictEqual(recorded.serverWrites, [
				[
					{
						label: "Prod",
						baseUrl: "http://prod.test",
						junk: "kept",
						headers: { "x-env": "gold", "x-count": 3, "x-flag": true },
					},
				],
			]);
			assert.strictEqual(recorded.syncRequests, 1, "the write reaches the engine like any servers-setting save");
			assert.strictEqual(recorded.parkedGlobalHeaders, undefined, "the parked record is consumed");
		});

		test("the entry's own header names win case-insensitively; skipped names ride the ack as a count", async () => {
			const recorded = makeEnv([{ label: "Prod", baseUrl: "http://prod.test", headers: { "X-Env": "keep-me" } }]);
			recorded.parkedGlobalHeaders = parked({ "x-env": "clobber", "x-new": "adds", "bad name": "x", "x-ctl": "a\nb" });
			...
			assert.deepStrictEqual(recorded.serverWrites, [
				[{ label: "Prod", baseUrl: "http://prod.test", headers: { "X-Env": "keep-me", "x-new": "adds" } }],
			]);
			assert.strictEqual(recorded.parkedGlobalHeaders, undefined);
			// Counts only, never the header names: the ack renders page-global.
			assert.strictEqual(typeof notice, "string");
			assert.ok(String(notice).includes("3"), String(notice));
			assert.ok(!String(notice).includes("x-env") && !String(notice).includes("bad name"), String(notice));
		});

		test("apply with nothing applicable still consumes the record without a settings write", async () => {
			const recorded = makeEnv([{ label: "Prod", baseUrl: "http://prod.test", headers: { "x-env": "keep" } }]);
			recorded.parkedGlobalHeaders = parked({ "X-ENV": "shadowed" });
			...
			assert.deepStrictEqual(recorded.serverWrites, [], "nothing to merge means no settings rewrite");
			assert.strictEqual(recorded.parkedGlobalHeaders, undefined, "the record is still consumed");
			assert.ok(String(notice).includes("1"), String(notice));
		});

		test("apply refuses an unknown label and an already-consumed record, touching nothing", async () => {
			const noEntry = makeEnv([]);
			noEntry.parkedGlobalHeaders = parked({ "x-env": "gold" });
			await assert.rejects(
				executeDashboardIntent(
					{ method: "resolveParkedHeaders", payload: { action: "apply", label: "Ghost" } },
					noEntry.env
				),
				/No servers setting entry/
			);
			assert.deepStrictEqual(noEntry.parkedGlobalHeaders, parked({ "x-env": "gold" }), "a refusal consumes nothing");

			const consumed = makeEnv([{ label: "Prod", baseUrl: "http://prod.test" }]);
			await assert.rejects(
				executeDashboardIntent(
					{ method: "resolveParkedHeaders", payload: { action: "apply", label: "Prod" } },
					consumed.env
				),
				/No parked headers remain/
			);
			assert.deepStrictEqual(consumed.serverWrites, []);
		});

		test("discard deletes the parked record without touching the setting, idempotently", async () => {
			const recorded = makeEnv([{ label: "Prod", baseUrl: "http://prod.test" }]);
			recorded.parkedGlobalHeaders = parked({ "x-env": "gold" });

			assert.strictEqual(
				await executeDashboardIntent({ method: "resolveParkedHeaders", payload: { action: "discard" } }, recorded.env),
				undefined
			);
			assert.strictEqual(recorded.parkedGlobalHeaders, undefined);
			assert.deepStrictEqual(recorded.serverWrites, []);

			// A second discard (a stale page) is a quiet no-op, never an error.
			await executeDashboardIntent({ method: "resolveParkedHeaders", payload: { action: "discard" } }, recorded.env);
			assert.strictEqual(recorded.parkedClears, 2);
		});

		test("a clear failing after the applied write reports an operation error, not a validation one", async () => {
			const recorded = makeEnv([{ label: "Prod", baseUrl: "http://prod.test" }]);
			recorded.parkedGlobalHeaders = parked({ "x-env": "gold" });
			recorded.failClearParked = new Error("memento write failed");
			...
			assert.strictEqual(recorded.serverWrites.length, 1, "the durable write landed before the failure");
		});
	});
});
