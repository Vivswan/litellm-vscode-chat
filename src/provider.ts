...
export class LiteLLMChatModelProvider implements LanguageModelChatProvider {
	...
	private _maxTokensFromModelInfo = new Map<string, boolean>();
	...
	async prepareLanguageModelChatInformation(
		...
		if (successfulCount > 0) {
			...
			this._maxTokensFromModelInfo.clear();
		}
		...
		for (let i = 0; i < results.length; i++) {
			...
			for (const [k, v] of reg.maxTokensFromModelInfo) {
				this._maxTokensFromModelInfo.set(k, v);
			}
