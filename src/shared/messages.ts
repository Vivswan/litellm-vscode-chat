...
/**
 * Apply an Anthropic ephemeral cache breakpoint to a message by tagging its last text content block.
 * String content is promoted to a single text block. Returns true if a breakpoint was placed.
 */
...
export function convertMessages(
	...
	// First-user-message breakpoint: tag the first user message so its prefix (tools + system + first user turn)
	// becomes a long-lived cacheable anchor that survives the entire agent session, even when the rolling-last
	...
	// Rolling conversation breakpoint: tag the last message so the entire prefix (system + tools + prior turns)
	// is cached and reused on the next agent round-trip. Anthropic allows up to 4 breakpoints; combined with
	// the system-prompt, last-tool, and first-user breakpoints this stays within budget. If the rolling tag
	// lands on the same message as the first-user tag, the helper is idempotent and still emits a single marker.
