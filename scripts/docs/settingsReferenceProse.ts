...
export const SETTING_PROSE: Readonly<Record<SettingId, SettingProse>> = {
	...
	"chat.additionalToolSchemaKeywords": {
		en:
			'Extra JSON-Schema keywords kept in tool input schemas, e.g. `["propertyNames"]`. Tool schemas are sanitized ' +
			"to a built-in keyword allowlist before sending; keywords listed here are kept too, values passed through " +
			"unchanged. The built-in set always applies. A keyword your server or model does not accept can fail requests " +
			"or degrade tool calling",
	...
	"chat.tokenEstimation": {
		en:
			'How prompt sizes are estimated for the local token budget: `"auto"` (script-aware heuristic that loads the ' +
			"o200k_base tokenizer when the VS Code display language is not English, or when the chat contains enough text " +
			'that plain character counting underestimates - CJK and other non-Latin scripts, emoji), `"heuristic"` (plain ' +
			'4-characters-per-token, never loads tokenizer data, undercounts CJK roughly 4x), `"o200k_base"` or ' +
			'`"cl100k_base"` (always load that tokenizer). A loaded tokenizer keeps roughly 10-30 MB in memory while ' +
			"active; counting cost is negligible",
		zhCn:
			'本地 token 预算如何估算提示大小: `"auto"` (识别文字系统的启发式, 在 VS Code 显示语言不是英语, 或聊天中含足够多让纯字符计数低估的文本 - CJK 等非拉丁文字、emoji - 时加载 ' +
			'o200k_base 分词器)、`"heuristic"` (纯粹按每 4 字符 1 token 计, 从不加载分词器数据, 对 CJK 低估约 4 倍)、`"o200k_base"` 或 ' +
			'`"cl100k_base"` (始终加载该分词器)。加载的分词器在活跃期间约占用 10-30 MB 内存; 计数开销可忽略',
		zhTw:
			'本地 token 預算如何估算提示大小: `"auto"` (識別文字系統的啟發式, 在 VS Code 顯示語言不是英文, 或聊天中含足夠多讓純字元計數低估的文字 - CJK 等非拉丁文字、emoji - 時載入 ' +
			'o200k_base 分詞器)、`"heuristic"` (純粹按每 4 字元 1 token 計, 從不載入分詞器資料, 對 CJK 低估約 4 倍)、`"o200k_base"` 或 ' +
			'`"cl100k_base"` (始終載入該分詞器)。載入的分詞器在活躍期間約佔用 10-30 MB 記憶體; 計數開銷可忽略',
	},
	"discovery.timeout": {
		en:
	...
	"discovery.staleServeWindow": {
		en:
			"How long a server that stops answering keeps serving its last known models flagged stale, counted from its " +
			"last successful discovery, in milliseconds. Raise it for a server that sleeps or restarts for longer than " +
			"ten minutes; `0` never serves stale models (a failed refresh empties that server's list at once). Details: " +
			"[Models - Discovery](models.md#discovery)",
	...
	"inlineCompletions.enabled": {
		en:
			"Opt-in for inline (ghost text) completions from a LiteLLM model, shipping with the inline completions " +
			"feature. Off by default: nothing is registered and nothing is sent until enabled, except the dashboard's " +
			'explicit "Test model" button, and enabling without `inlineCompletions.model` keeps the feature idle',
	...
	"inlineCompletions.languageFilter": {
		en:
			'Where inline completions run: mode `"block"` runs them everywhere except the listed exact VS Code language ' +
			'IDs, `"allow"` only in the listed ones (an empty allow list runs nowhere), e.g. `{ "mode": "block", ' +
			'"languages": ["markdown", "plaintext"] }`. The default blocks nothing',
	...
	"commitGeneration.enabled": {
		en:
			"Opt-in for commit message generation from a LiteLLM model, shipping with the commit generation feature. Off " +
			"by default: the command stays hidden and nothing is sent until enabled, except the dashboard's explicit " +
			'"Test model" button, and enabling without `commitGeneration.model` keeps the feature idle',
	...
	"prGeneration.enabled": {
		en:
			"Opt-in for pull request title and description generation from a LiteLLM model. Off means the command stays " +
			'hidden and no requests are sent, except the dashboard\'s explicit "Test model" button ([the ' +
			"recipe](getting-started.md#generate-pull-request-descriptions-with-your-own-model))",
	...
	"consultTool.enabled": {
		en:
			"Opt-in for the consult tool, which lets a chat agent ask a second LiteLLM model for its opinion. Off by " +
			"default: nothing is registered and nothing is sent until enabled, except the dashboard's explicit \"Test " +
			'model" button, and enabling without `consultTool.model` keeps the tool unregistered. Once both are set the ' +
			"agent calls it on its own initiative",
	...
	"quickFix.enabled": {
		en:
			"Show Fix and Explain quick fixes on diagnostics. Both open the chat view and send the question to " +
			"`@litellm`, with the diagnostics and the lines they sit on attached; when the chat view cannot answer they " +
			"fall back to `quickFix.model`. See [the quick fix recipe](getting-started.md#fix-or-explain-a-diagnostic)",
