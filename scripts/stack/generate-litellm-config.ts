...
// LiteLLM config to stdout WITHOUT touching docker/.generated/, so it can
// never desync the file on disk from the config a running container loaded.
// In the environment-honoring mode a seeded Copilot login makes it fetch the
// live catalog from GitHub, so it can take seconds and needs the network.
//
// Usage:
